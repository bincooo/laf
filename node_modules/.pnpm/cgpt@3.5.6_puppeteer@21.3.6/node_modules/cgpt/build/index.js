...
async function browserPostEventStream(url2, accessToken, body, timeoutMs, enabledParser) {
  ...
  try {
      ...
      async (resolve, reject) => {
        async function onMessage(data) {
          ...
          try {
            ...
            const { message_type, model_slug } = convoResponseEvent.message?.metadata ?? {};
            if (partialResponse && message_type === "next") {
        ...
        try {
          ...
          console.log("chatgpt-apt-browser 1 ===>>> ", err);
          reject(err);
        }
      }
    );
    ...
    console.log("chatgpt-apt-browser 2 ===>>> ", err);
    abortController.abort();
...
var ChatGPTAPIBrowser = class extends AChatGPTAPI {
  ...
  async getCfClearance(refresh = false) {
    if (refresh)
      await this.refreshSession();
    let count = 10;
    while (!this._cfClearance && count-- > 0) {
      await delay3(3e3);
    }
    return this._cfClearance;
  }
  async getAccessToken(refresh = false) {
    if (refresh)
      this.refreshSession();
    let count = 10;
    while (!this._accessToken && count-- > 0) {
      await delay3(3e3);
    }
